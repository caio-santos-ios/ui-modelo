...
export const CustomerModalMinimalCreate = () => {
    ...
    const create: SubmitHandler<TCustomer> = async (body: TCustomer) => {
        try {
            setLoading(true);
            const {data} = await api.post(`/customers/minimal`, body, configApi());
            resolveResponse({status: 201, message: data.result.message});
            closeModal();
            setCustomer(data.result.data ?? ResetCustomer);
        } catch (error) {
    ...
    const update: SubmitHandler<TCustomer> = async (body: TCustomer) => {
        try {
            setLoading(true);
            const {data} = await api.put(`/customers/minimal`, body, configApi());
